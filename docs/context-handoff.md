...
- `/rooms` 카드 레이아웃 compact 정리
- 로비 채팅 자동 스크롤 적용
- Phaser 리마운트로 인한 게임 화면 깜빡임 수정
- `Q`를 길게 눌러도 최초 1회만 발사되도록 수정
- 피격/발사 피드백 1차 polish 추가
- `guest`, `rooms`, `room`, `game` 주요 문구 i18n 적용
- Phaser 씬 문구를 locale 기반으로 주입하는 구조로 정리
- `*.tsbuildinfo` 파일이 git 변경으로 잡히지 않도록 `.gitignore` 정리

## 현재 동작하는 수준

현재는 **MVP 코어 완성 + 1차 polish/i18n 진행 상태**입니다.
...
- 결과 연출 강화
- HUD 레이아웃 정리
- alert / prompt 기반 UX를 모달/토스트로 교체

3. 네트워크 / 감각 튜닝
- 이동 보간 고도화
- 투사체/이동 수치 재조정
- disconnect / reconnect 예외 처리 보강
...
- `Q` 길게 누를 때 1회만 발사되는지 테스트

## 다음 코덱스 스레드에 바로 요청하기 좋은 문장 예시

- `docs/context-handoff.md를 읽고 도트 아트 에셋 구조와 캐릭터/식칼 스프라이트 반영부터 진행해줘.`
- `docs/context-handoff.md를 읽고 남은 i18n 하드코드를 정리하고 alert/prompt UX를 모달로 바꿔줘.`
- `docs/context-handoff.md를 읽고 HUD 시각 polish와 결과 연출을 강화해줘.`

## 참고

- 현재 `pnpm typecheck`는 통과 상태여야 정상
- 스프린트 3까지 코어 기능은 구현 완료
- 이후 커밋으로 i18n, 버그 수정, 전투 polish가 추가 반영됨
